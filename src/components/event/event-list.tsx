...
export function EventList({ initialEvents, canWrite, canDelete = false, loadError = false }: { initialEvents: EventListItem[]; canWrite: boolean; canDelete?: boolean; loadError?: boolean }) {
  ...
  const events = useMemo(() => {
    ...
    const filtered = eventItems.filter((event) => {
      const matchesSearch = !deferredSearch || `${event.name} ${event.venue}`.toLowerCase().includes(deferredSearch.toLowerCase());
  ...
  }, [dateFilter, deferredSearch, eventItems, sort, status]);
  ...
  async function cancelEvent() {
    ...
    try {
      ...
      if (!response.ok) {
        setError(body.error ?? "ไม่สามารถยกเลิก Event ได้");
        return;
      }
      const nextStatus: EventListItem["status"] = body.event?.status ?? "CANCELLED";
      setEventItems((current) => current.map((event) => event.id === selected.id ? { ...event, status: nextStatus } : event));
      setSelected(null);
    } catch {
      setError("ไม่สามารถเชื่อมต่อเพื่อยกเลิก Event ได้");
    } finally {
  ...
  return <>
    ...
    <Card>
      <CardContent>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-line pt-4">{loadError ? <p className="text-sm text-muted">ยังไม่สามารถสรุปจำนวนกิจกรรมได้</p> : <p className="text-sm text-muted">พบ <strong className="text-ink">{events.length}</strong> จาก {eventItems.length} กิจกรรม</p>}{hasFilters ? <button type="button" className="focus-ring inline-flex min-h-10 items-center gap-1.5 rounded-sm px-2 text-sm font-semibold text-link hover:bg-[#eef6ff]" onClick={clearFilters}><X size={15} />ล้างตัวกรอง</button> : <span className="inline-flex items-center gap-1.5 text-xs text-muted"><SlidersHorizontal size={14} />กรองข้อมูลได้ทันที</span>}</div>
      </CardContent>
    </Card>

    <div className="mt-5">
      {events.length === 0 ? <Card><CardContent><EmptyState title={loadError ? "ยังไม่สามารถแสดงรายการกิจกรรม" : hasFilters ? "ไม่พบกิจกรรมที่ตรงเงื่อนไข" : "ยังไม่มีกิจกรรม"} description={loadError ? "ระบบยังไม่พร้อมเชื่อมต่อฐานข้อมูล โปรดลองใหม่อีกครั้ง" : hasFilters ? "ลองเปลี่ยนคำค้นหาหรือล้างตัวกรองเพื่อดูข้อมูลทั้งหมด" : `เริ่มต้นด้วยการตั้งค่ากิจกรรม ${PRODUCT_NAME}`} action={canWrite && !hasFilters && !loadError ? <Button href="/admin/events/new">สร้างกิจกรรม</Button> : undefined} /></CardContent></Card> : view === "card" ? <div className="grid gap-5 md:grid-cols-2">{events.map((event) => <EventCard key={event.id} event={event} />)}</div> : <Card><CardContent className="p-0"><div className="table-scroll"><table className="data-table"><thead><tr><th>กิจกรรม</th><th>วันเวลา</th><th>ผู้เข้าร่วม</th><th>Check-in</th><th>สถานะ</th><th className="text-right">การทำงาน</th></tr></thead><tbody>{events.map((event) => <tr key={event.id}><td><Link href={`/admin/events/${event.id}`} className="flex min-w-[240px] items-center gap-3 font-semibold hover:text-primary"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-navy text-white"><CalendarDays size={17} /></span><span><span className="block truncate">{event.name}</span><span className="mt-0.5 block max-w-[240px] truncate text-xs font-normal text-muted">{event.venue}</span></span></Link></td><td className="whitespace-nowrap text-sm text-muted">{formatBangkokDateTime(event.startAt)}</td><td className="mono text-sm">{event.registered.toLocaleString("th-TH")}</td><td className="mono text-sm">{event.checkedIn.toLocaleString("th-TH")} / {event.registered.toLocaleString("th-TH")}</td><td><StatusBadge status={event.status} label={statusLabels[event.status] ?? event.status} /></td><td><div className="flex justify-end gap-1"><Button href={`/admin/events/${event.id}`} variant="ghost" size="sm">เปิด</Button>{canWrite ? <details className="relative"><summary className="focus-ring touch-target list-none rounded-md text-muted hover:bg-paper"><MoreHorizontal size={18} /><span className="sr-only">เมนูกิจกรรม {event.name}</span></summary><div className="absolute right-0 z-10 mt-1 w-44 rounded-lg border border-line bg-white p-1 shadow-soft"><Link className="block rounded-md px-3 py-2 text-sm hover:bg-paper" href={`/admin/events/${event.id}/edit`}>แก้ไขกิจกรรม</Link>{event.status !== "CANCELLED" ? <button type="button" className="flex min-h-11 w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm text-danger hover:bg-[#fff4f4]" onClick={() => setSelected(event)}><Trash2 size={15} />ยกเลิกกิจกรรม</button> : null}{canDelete ? <DeleteEventButton eventId={event.id} eventName={event.name} menuItem /> : null}</div></details> : null}</div></td></tr>)}</tbody></table></div></CardContent></Card>}
    </div>
