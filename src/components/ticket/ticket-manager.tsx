...
function TicketSummary({ label, value, icon: Icon, tone = "primary", detail }: { label: string; value: number; icon: typeof TicketIcon; tone?: "primary" | "success"; detail?: string }) { return <Card className="p-4"><div className="flex items-center justify-between gap-3"><span className={`grid h-9 w-9 place-items-center rounded-lg ${tone === "success" ? "bg-[#eaf5f0] text-success" : "bg-[#eaf6f3] text-primary"}`}><Icon size={18} /></span><span className="mono text-2xl font-semibold">{value.toLocaleString("th-TH")}</span></div><p className="mt-3 text-sm font-semibold">{label}</p>{detail ? <p className="mt-1 text-xs text-muted">{detail}</p> : null}</Card>; }