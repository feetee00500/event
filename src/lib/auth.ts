...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  pages: { signIn: "/login" },
  providers: [Credentials({ credentials: { email: { label: "Email", type: "email" }, password: { label: "Password", type: "password" } }, async authorize(credentials) { const parsed = loginSchema.safeParse(credentials); if (!parsed.success) return null; const db = getDb(); const user = await db.user.findUnique({ where: { email: parsed.data.email.toLowerCase() } }); if (!user?.isActive || !user.passwordHash) return null; const valid = await bcrypt.compare(parsed.data.password, user.passwordHash); if (!valid) return null; await db.user.update({ where: { id: user.id }, data: { lastLoginAt: new Date() } }); return { id: user.id, name: user.name, email: user.email, role: user.role }; } })],
  callbacks: {
...
async function resolveCurrentUser(): Promise<CurrentUser | null> {
  if (isDevelopmentAuthBypassEnabled()) {
    ...
    try {
      const db = getDb();
      const user = await db.user.findFirst({ where: { role: "SUPER_ADMIN", isActive: true }, orderBy: { createdAt: "asc" } });
