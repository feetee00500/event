...
const mocks = vi.hoisted(() => ({
  attendeeCount: vi.fn(), checkinCount: vi.fn(), gateFindMany: vi.fn(), ticketGroupBy: vi.fn(), checkinFindMany: vi.fn(),
}));
...
beforeEach(() => {
  ...
  mocks.checkinFindMany.mockReset().mockResolvedValue(Array.from({ length: 9 }, (_, index) => ({ scannedAt: new Date(`2026-08-04T0${index % 2}:00:00.000Z`) })));
});
