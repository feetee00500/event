...
export async function processCheckin(input: CheckinInput): Promise<CheckinResponse> {
  ...
  const [event, ticket] = await Promise.all([
    db.event.findUnique({ where: { id: input.eventId }, select: { id: true, status: true, accessMode: true, checkinOpenAt: true, checkinCloseAt: true, gates: { where: { id: input.gateId, isActive: true }, take: 1, select: { id: true, name: true } } } }),
    db.ticket.findUnique({ where: input.ticketNumber ? { ticketNumber: input.ticketNumber } : { qrTokenHash: tokenHash }, select: { id: true, eventId: true, attendeeId: true, ticketNumber: true, ticketType: true, status: true, expiresAt: true, attendee: { select: { id: true, firstName: true, lastName: true } } } }),
  ]);
  ...
  if (!ticket) {
    await db.checkin.create({ data: checkinLogData(input, event.id, "INVALID_TOKEN", meta, undefined, gate.id) });
    return failure("INVALID_TOKEN");
  }
  if (ticket.eventId !== event.id) {
    await db.checkin.create({ data: checkinLogData(input, event.id, "EVENT_MISMATCH", meta, ticket.id, gate.id) });
    return failure("EVENT_MISMATCH");
  }
  const windowResult = isWithinWindow(now, event.checkinOpenAt, event.checkinCloseAt);
  if (windowResult) {
    await db.checkin.create({ data: checkinLogData(input, event.id, windowResult, meta, ticket.id, gate.id) });
  ...
  if (ticketExpired && ticket.status === "ACTIVE") await db.ticket.update({ where: { id: ticket.id }, data: { status: "EXPIRED" } });
  if (ticketExpired || ticket.status === "EXPIRED") {
    await db.checkin.create({ data: checkinLogData(input, event.id, "EXPIRED", meta, ticket.id, gate.id) });
    return failure("EXPIRED");
  }
  if (ticket.status === "CANCELLED") {
    await db.checkin.create({ data: checkinLogData(input, event.id, "CANCELLED", meta, ticket.id, gate.id) });
    return failure("CANCELLED");
  }
  if (ticket.status === "CHECKED_IN" && event.accessMode !== "REENTRY") {
    const [firstCheckin] = await Promise.all([
      db.checkin.findFirst({ where: { ticketId: ticket.id, result: "SUCCESS" }, orderBy: { scannedAt: "asc" }, select: { scannedAt: true, gate: { select: { name: true } } } }),
      db.checkin.create({ data: checkinLogData(input, event.id, "ALREADY_CHECKED_IN", meta, ticket.id, gate.id) }),
    ]);
  ...
  const rows = await db.$queryRaw<{ claimed: number }[]>`
    WITH claimed AS (
  ...
  if (rows[0]?.claimed !== 1) {
    const [latest, firstCheckin] = await Promise.all([
      db.ticket.findUnique({ where: { id: ticket.id }, select: { status: true } }),
      db.checkin.findFirst({ where: { ticketId: ticket.id, result: "SUCCESS" }, orderBy: { scannedAt: "asc" }, select: { scannedAt: true, gate: { select: { name: true } } } }),
    ]);
    if (latest?.status === "CANCELLED") return failure("CANCELLED");
    if (latest?.status === "EXPIRED") return failure("EXPIRED");
    await db.checkin.create({ data: checkinLogData(input, event.id, "ALREADY_CHECKED_IN", meta, ticket.id, gate.id) });
    return failure("ALREADY_CHECKED_IN", { firstCheckedInAt: firstCheckin?.scannedAt.toISOString(), gateName: firstCheckin?.gate?.name });
  }
