...
export default async function DashboardPage() {
  ...
  const project = hasError ? undefined : data.latestEvents[0];
  const canConfigure = hasPermission(user.role, "events:write");
  const projectHref = project ? `/admin/events/${project.id}` : hasError ? "/admin/events" : canConfigure ? "/admin/events/new" : "/admin/events";
  const workspaceHref = project ? `/admin/events/${project.id}` : "/admin/events";
  const workspaceDescription = hasError ? "โหลดข้อมูลกิจกรรมไม่สำเร็จ ลองใหม่อีกครั้ง" : project ? "เปิด Workspace ของกิจกรรมนี้" : "เลือกกิจกรรมเพื่อเปิด Workspace";
