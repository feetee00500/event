...
export default async function CheckinsPage({ params }: Props) {
  ...
  try {
    const [event, checkins] = await Promise.all([
      ...
      getCheckins(user, eventId),
    ]);
    if (!event) return <><PageHeader eyebrow="Event" title="ประวัติ Check-in" description="ตรวจสอบผลการสแกนล่าสุด ค้นหาความผิดปกติ และกรองตาม Gate หรือ Staff" /><EventTabs eventId={eventId} active="/checkins" canManage={false} /><div className="mt-6"><InlineNotice tone="error">ไม่พบ Event</InlineNotice></div></>;
