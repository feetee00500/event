...
export default async function CheckinsPage({ params }: Props) {
  ...
  try {
    const [event, checkins] = await Promise.all([getEvent(user, eventId), getCheckins(user, eventId)]);
    ...
    return <><PageHeader eyebrow={event.name} title="ประวัติ Check-in" description="ตรวจสอบผลการสแกนล่าสุด ค้นหาความผิดปกติ และกรองตาม Gate หรือ Staff" /><EventTabs eventId={eventId} active="/checkins" /><div className="mt-6"><CheckinHistory initialRows={rows} /></div></>;
  } catch {
