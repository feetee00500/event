...
export default async function TicketsPage({ params }: Props) {
  ...
  try {
    const [event, tickets] = await Promise.all([getEvent(user, eventId), getTickets(user, eventId)]);
    ...
    return <><PageHeader eyebrow={event.name} title="บัตรและ QR Code" description={`ออกบัตรดิจิทัลในรูปแบบ ${PRODUCT_NAME} พร้อม QR สำหรับ Check-in`} /><EventTabs eventId={eventId} active="/tickets" /><div className="mt-6"><TicketManager event={{ name: event.name, startAt: event.startAt.toISOString(), endAt: event.endAt.toISOString(), venue: event.venue }} initialTickets={rows} canWrite={user.role === "SUPER_ADMIN" || user.role === "EVENT_ADMIN"} /></div></>;
  } catch {
