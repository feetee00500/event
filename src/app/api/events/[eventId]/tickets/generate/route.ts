...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const attendees = await prisma.attendee.findMany({ where: { eventId, ...(attendeeIds.length ? { id: { in: attendeeIds } } : {}) }, take: MAX_TICKETS_PER_REQUEST, include: { tickets: { orderBy: { createdAt: "desc" }, take: 1 } } });
