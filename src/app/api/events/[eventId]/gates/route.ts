...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    const gate = await prisma.gate.create({ data: { eventId, name: data.name, description: data.description || null, location: data.location || null, isActive: data.isActive, deviceCode: createDeviceCode() } });
    await prisma.auditLog.create({ data: { userId: user.id, eventId, action: "GATE_CREATED", entityType: "Gate", entityId: gate.id, newValue: { name: gate.name } } });
