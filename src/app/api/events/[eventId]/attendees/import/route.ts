...
export async function POST(request: Request, { params }: Context) {
  try {
    ...
    for (const [index, row] of rows.entries()) {
      ...
      if (!parsed.success) {
        errors.push({ row: index + 2, message: Object.values(parsed.error.flatten().fieldErrors).flat().join(", ") || "ข้อมูลไม่ครบ" });
        continue;
      }
      try {
        ...
        errors.push({ row: index + 2, message: "ไม่สามารถบันทึกข้อมูลแถวนี้ได้ อาจมีเลขอ้างอิงซ้ำ" });
      }
    }
