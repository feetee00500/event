...
export default async function ScannerPage({ params }: Props) {
  ...
  try {
    ...
    return <div className="mx-auto max-w-2xl"><Link href="/scanner" className="focus-ring mb-5 inline-flex items-center gap-2 text-sm font-semibold text-muted hover:text-primary"><ArrowLeft size={16} />เลือก Event อื่น</Link><ScannerClient eventId={event.id} eventName={event.name} gates={gates} /></div>;
  } catch {
